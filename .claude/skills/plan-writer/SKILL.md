...
#### Phase 1：数据准备（并行 3 Agent）

执行前，主流程先从 profile.json 提取精简摘要（不传原始工商数据）：
...
**Step 1**：启动 collect-and-validate（必须先完成，后续 Agent 依赖其输出）

| Agent | Prompt 文件 | 注入数据 |
|-------|------------|---------|
| collect-and-validate | `prompts/collect-and-validate.md` | 原始文件路径 |

**⚠️ 门控检查**：检查返回的 `gate` 字段：
- `gate = "blocked"` → **停止执行**，将 reason 输出给用户
- `gate = "pass_with_warnings"` → 继续，但将 warnings 记录
- `gate = "pass"` → 继续

**Step 2**：门控通过后，并行启动 2 个 Agent

| Agent | Prompt 文件 | 触发条件 | 注入数据 |
|-------|------------|---------|---------|
| match-knowledge-base | `prompts/match-knowledge-base.md` | 始终执行 | `{profile_summary}` + index.json 路径 + `{data_summary}` 中的需求关键词 |
| competitive-positioning | `prompts/competitive-positioning.md` | sales-guide.json 存在时 | sales-guide.json 路径 + `{data_summary}` 中的需求摘要 |

#### Phase 2：A/B 方案生成（并行 2 Agent）

Phase 1 全部完成后，同时启动 2 个 Agent：

| Agent | Prompt 文件 | 注入数据 |
|-------|------------|---------|
| build-plan-a | `prompts/build-plan-a.md` | `{data_summary}` + `{knowledge_matches}` + `{competitive_strategy}` + output-template.md |
| critique-plan-b | `prompts/critique-plan-b.md` | `{data_summary}` + `{knowledge_matches}` + `{competitive_strategy}` |

**关键设计**：Plan B 不依赖 Plan A 的输出。B 是独立批判分析，给相同输入从审慎角度生成风险和替代方案。真正的 A/B 融合在 Phase 3 完成。

#### Phase 3：专家评审（1 Agent）

Phase 2 全部完成后，启动 1 个 Agent：

| Agent | Prompt 文件 | 注入数据 |
|-------|------------|---------|
| expert-review | `prompts/expert-review.md` | `{plan_a}` + `{critique_b}` + `{data_summary}` + output-template.md |

expert-review 输出 `{final_content}`：完整的 Markdown 方案文档。
...
6. 向用户展示方案内容
7. 展示 `pendingConfirmations`（待客户确认项）
...
#### Phase 1：知识库匹配（并行 2 Agent）

| Agent | Prompt 文件 | 注入数据 |
|-------|------------|---------|
| match-knowledge-base | `prompts/match-knowledge-base.md` | `{dialog_extract}` 中的行业/需求关键词 + index.json 路径 |
| competitive-positioning | `prompts/competitive-positioning.md` | sales-guide.json 路径（如有） + `{dialog_extract}` 中的需求 |
...
**上次是 Mode A 生成的**：
   - requirements.json 版本变化 → 重跑 Phase 1 + Phase 2 + Phase 3
   - 仅约束变化 → 重跑 collect-and-validate + expert-review（注入新约束 + 现有方案）
...
└── prompts/
    ...
    ├── collect-and-validate.md    # Mode A Phase 1: 数据收集 + 质量门控
    ├── match-knowledge-base.md    # Phase 1: 知识库加权匹配（A/C 共用）
    ├── competitive-positioning.md # Phase 1: 竞争定位分析（A/C 共用）
    ├── build-plan-a.md            # Phase 2: 建设者视角方案（A/B/C 共用）
    ├── critique-plan-b.md         # Phase 2: 批判者视角分析（A/B/C 共用）
    └── expert-review.md           # Phase 3: 专家综合评审（A/B/C 共用）
```
